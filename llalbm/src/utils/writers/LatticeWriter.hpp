...
namespace llalbm::util::writer
{
    ...
    void create_output_directory()
    {
        ...
        }else
        {
            ...
            logger.info("clearing the directory (only .bin files)");
    ...
    {
        std::string output_file_path_u_x = output_dir_path + "/output-" + std::to_string(iteration_count) + "u_x.bin";
        std::ofstream output_file_u_x(output_file_path_u_x, std::ios::binary);

        std::string output_file_path_u_y = output_dir_path + "/output-" + std::to_string(iteration_count) + "u_y.bin";
        std::ofstream output_file_u_y(output_file_path_u_y, std::ios::binary);
        ...
        for (Eigen::Index i = 0; i < global_u.dimension(0); ++i) {
            for (Eigen::Index j = 0; j < global_u.dimension(1); ++j) {
                output_file_u_x.write(reinterpret_cast<const char*>(&global_u(i, j, 0)), sizeof(double));
                output_file_u_y.write(reinterpret_cast<const char*>(&global_u(i, j, 1)), sizeof(double));
            }
        }
