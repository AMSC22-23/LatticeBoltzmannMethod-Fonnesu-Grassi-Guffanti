...
namespace llalbm::core::boundaries
{
    ...
    template<
        std::size_t dim>
    class CUDAPSBounceBackPolicy : public BoundaryPolicyTag, public CUDATag{
    private:
        /* data */
        Logger log;
    public:
        CUDAPSBounceBackPolicy(/* args */)
        : log("Partially Saturated BB" , std::cout)
        {
            log.error("Partially Saturated BB not implemented");
            assert(false);
    ...
    template<>
    class CUDAPSBounceBackPolicy<2> : public BoundaryPolicyTag, public CUDATag{
    ...
    // initialization of the relaxation constant in the 2-D PS Bounce-Back.
    double CUDAPSBounceBackPolicy<2>::tau = 0.0;
}
