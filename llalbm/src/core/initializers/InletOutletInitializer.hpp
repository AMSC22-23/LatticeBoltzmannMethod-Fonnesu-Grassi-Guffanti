...
template<std::size_t dim>
    ...
    template<std::size_t dim>
    class CUDAVelocityInitializer : public InitializationPolicyTag, public CUDATag
    {
    public: 
        static std::vector<BoundaryPoint<dim>> inlet_nodes;
        static std::vector<BoundaryPoint<dim>> outlet_nodes;
         ...
         * thought as being completely static
         */
        CUDAVelocityInitializer() = default;
        ...
        static void update_macro(const Tensor<double, 3> &populations, const std::vector<Point<2>> &fluid_nodes, Tensor<double, 2> &global_rho, Tensor<double, 3> &global_u)
        {
            update_macro(populations, fluid_nodes, global_rho, global_u);
        }
    ...
    template<std::size_t dim>
    std::vector<BoundaryPoint<dim>> CUDAVelocityInitializer<dim>::inlet_nodes;
    template<std::size_t dim>
    std::vector<BoundaryPoint<dim>> CUDAVelocityInitializer<dim>::outlet_nodes;
    template<std::size_t dim>
    std::array<std::function<double (double, BoundaryPoint<dim>)>, dim> CUDAVelocityInitializer<dim>::inlet_update_function;
    template<std::size_t dim>
    std::array<std::function<double (double, BoundaryPoint<dim>)>, dim> CUDAVelocityInitializer<dim>::outlet_update_function;
