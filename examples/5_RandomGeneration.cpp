...
int main()
{
    ...
    info.attach_domain_dimensions({100, 700});
    ...
    info.add_nodes_interval({1, 699}, {98, 699}, NonFluidNodeType::OUTLET);
    // To add random spheres we just need to call the add_obstacle_random_spheres function
    info.add_obstacle_random_spheres(15, 20, 8, 69210);
    ...
    std::ofstream out("RandomStructure.txt");
    lattice.print_lattice_structure(out);

    // Setting up the collider and initializer
    collisions::OMPTRTCollisionPolicy<2>::initialize(0.58, 1, 1.0/std::sqrt(3.0));
