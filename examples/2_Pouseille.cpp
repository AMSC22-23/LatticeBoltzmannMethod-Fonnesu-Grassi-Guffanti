...
int main()
{
    ...
    std::ofstream out("ObstacleGenerations.txt");
    lattice.print_lattice_structure(out);

    // Setting up the collider and initializer
    collisions::OMPTRTCollisionPolicy<2>::initialize(0.9, 1.0, 1.0/std::sqrt(3.0));
    collisions::OMPTRTCollisionPolicy<2>::enforce_magic_parameter(1.0/6.0);
