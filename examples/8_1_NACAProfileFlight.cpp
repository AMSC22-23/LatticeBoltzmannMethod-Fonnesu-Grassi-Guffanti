...
/**
 * @brief In this eighth example we will simulate the flow around a NACA profile, in a mid-flight context.
...
int main()
{
    ...
    info.read_obstacle_from_file("../examples/NACA-foil.txt");
    ...
    std::ofstream out("WingStructure2.txt");
    lattice.print_lattice_structure(out);

    // Setting up the collider and initializer
    {
        collisions::OMPTRTCollisionPolicy<2>::initialize(0.9, 1.2, 1.0/std::sqrt(3.0));
