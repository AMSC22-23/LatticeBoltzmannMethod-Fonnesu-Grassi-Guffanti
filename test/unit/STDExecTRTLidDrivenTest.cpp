...
int main()
{
    ...
    using Config = LatticeConfiguration<
        2,
        collisions::STDExecTRTCollisionPolicy<2>,
        boundaries::STDExecBounceBackPolicy<2>,
        boundaries::STDExecBounceBackPolicy<2>,
        boundaries::STDExecZouHePolicy<2>,
        boundaries::STDExecZouHePolicy<2>, 
        initializers::STDExecVelocityInitializer<2>,
        equilibrium::STDExecDefaultEquilibrium<2>
    >;   

    using Parallel = STDExecPolicy<2, Config>;
    ...
    initializers::STDExecVelocityInitializer<2>::attach_update_functions(VelocityFunctions,Outlets);

    collisions::STDExecTRTCollisionPolicy<2>::initialize(0.9, 0.01, 1./std::sqrt(3.0));
    collisions::STDExecTRTCollisionPolicy<2>::compute_magic_parameter();
    collisions::STDExecTRTCollisionPolicy<2>::enforce_magic_parameter(1.0/4.0);
