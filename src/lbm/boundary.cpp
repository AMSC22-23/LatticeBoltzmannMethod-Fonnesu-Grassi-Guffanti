...
{ 
    ...
    node.set_rho() = (f[0] + f[1] + f[3] + 2.0 * (f[2] + f[5] + f[7])) / (1.0 + node.get_u()[1]);

    node.set_population(4) = f[2] - two_thirds * node.get_rho() * node.get_u()[1];

    node.set_population(8) = f[6] - one_half*(f[1] - f[3]) +
                             one_half * node.get_rho() * node.get_u()[0] -
                             one_sixth * node.get_rho() * node.get_u()[1];
...
void NEBB::calc_bottom_wall_bounce(LatticeNode<2> &node) const
{
    ...
    node.set_rho() = (f[0] + f[1] + f[3] + 2.0 * (f[4] + f[7] + f[8])) / (1.0 - node.get_u()[1]);

    // const double ru = rho * node.get_u()[1];

    node.set_population(2) = f[4] + two_thirds * node.get_rho() * node.get_u()[1];
...
void NEBB::calc_left_wall_bounce(LatticeNode<2> &node) const
{
    ...
    node.set_rho() = (f[0] + f[2] + f[4] + 2.0 * (f[3] + f[7] + f[6])) / (1.0 - node.get_u()[0]);

    //const double ru = rho * node.get_u()[0];

    node.set_population(1) = f[3] + two_thirds * node.get_rho() * node.get_u()[0];

    node.set_population(5) = f[7] - one_half*(f[2]-f[4]) + 
                             one_sixth * node.get_rho() * node.get_u()[0] + 
...
void NEBB::calc_right_wall_bounce(LatticeNode<2> &node) const
{
    ...
    node.set_rho() = (f[0] + f[2] + f[4] + 2.0 * (f[1] + f[5] + f[8])) / (1.0 + node.get_u()[0]);

    //const double ru = rho * node.get_u()[0];

    node.set_population(3) = f[1] - two_thirds * node.get_rho() * node.get_u()[0];
