...
void NEBB::calc_top_wall_bounce(LatticeNode<2> &node) const
{
    ...
    const double rho = (f[0] + f[1] + f[3] + 2.0 * (f[2] + f[5] + f[6])) / (1.0 + node.get_u()[1]);

    const double ru = rho * node.get_u()[1];

    node.set_population(4) = f[2] - two_thirds * ru;
    node.set_population(7) = f[5] - one_sixth * ru + one_half * (f[1] - f[3]);
    node.set_population(8) = f[6] - one_sixth * ru + one_half * (f[3] - f[1]);
}
...
void NEBB::calc_bottom_wall_bounce(LatticeNode<2> &node) const
{
    ...
    node.set_population(2) = f[4] + two_thirds * ru;
    node.set_population(5) = f[7] + one_sixth * ru - one_half * (f[1] - f[3]);
    node.set_population(6) = f[8] + one_sixth * ru - one_half * (f[3] - f[1]);
}
...
void NEBB::calc_left_wall_bounce(LatticeNode<2> &node) const
{
    ...
    node.set_population(1) = f[3] + two_thirds * ru;
    node.set_population(5) = f[7] + one_sixth * ru - one_half * (f[2] - f[4]);
    node.set_population(8) = f[6] + one_sixth * ru - one_half * (f[4] - f[2]);
}
...
void NEBB::calc_right_wall_bounce(LatticeNode<2> &node) const
{
    ...
    node.set_population(3) = f[1] - two_thirds * ru;
    node.set_population(7) = f[5] - one_sixth * ru + one_half * (f[2] - f[4]);
    node.set_population(6) = f[8] - one_sixth * ru + one_half * (f[4] - f[2]);
}
